...
const ProductsShowcase: React.FC = () => {
  ...
  const ProductCard: React.FC<{ product: Product; index: number }> = ({ product, index }) => {
    ...
    return (
      <div 
        ...
        className={`group relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border-2 border-transparent hover:border-amber-200 transform hover:-translate-y-2 ${
      ...
      >
        ...
        {/* Animated Border */}
        <div className="absolute inset-0 rounded-3xl border-2 border-transparent group-hover:border-amber-400 transition-all duration-500 pointer-events-none"></div>
      </div>
    );
  };
  ...
  const ProductListItem: React.FC<{ product: Product }> = ({ product }) => {
    return (
      <div className="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border-2 border-transparent hover:border-amber-200 p-6">
  ...
  return (
    <div 
      ref={sectionRef}
      className="relative bg-gradient-to-br from-slate-50 via-white to-amber-50/30 py-32 overflow-hidden"
    >
      {/* Background Pattern */}
      ...
      {/* Decorative Blobs */}
      <div className="absolute top-20 right-20 w-96 h-96 bg-amber-200 rounded-full blur-3xl opacity-20 animate-pulse"></div>
      <div className="absolute bottom-40 left-20 w-80 h-80 bg-slate-200 rounded-full blur-3xl opacity-30 animate-pulse delay-1000"></div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Search & Filter Bar */}
        <div className={`bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl border-2 border-slate-200 p-8 mb-12 transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        {/* Results Count */}
        {!loading && (
          <div className={`mb-8 transition-all duration-1000 delay-400 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
        ...
        {!loading && (!Array.isArray(filteredProducts) || filteredProducts.length === 0) && (
          <div className="text-center py-20">
            <div className="w-24 h-24 bg-gradient-to-br from-slate-200 to-amber-200 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-lg">
